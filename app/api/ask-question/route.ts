...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    //Step 2: Search the vectore store for relevant emalis using similarity search
    const { data: matchingSections, error } = await supabase.rpc(
      "match_filtered_email_sections",
      {
        query_embedding: questionEmbedding,
        match_threshold: -0.3,
        match_count: 10, //Number of emails to retrive
        email_address: "nursultan.seyhan@gmail.com", //Add the filter for a spesific mail
      },
    );
